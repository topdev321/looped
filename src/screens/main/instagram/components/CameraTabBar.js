...
class TabBar<T: *> extends React.Component<Props<T>, State> {
  static propTypes = {
  ...
  _handleTabPress = ({ route }) => {
    ...
    !route.isVideoTab && this.props.jumpTo(route.key);
  };
  ...
  _renderTabButton = (route, i) => {
    const { position, navigationState, scrollEnabled } = this.props;
    const { routes, cameraTabIndex } = navigationState;
    const inputRange = [-1, ...routes.map((x, i) => i), 2];
    ...
    if (this.props.videoFocused) {
      tabStyle.opacity = new Animated.Value(route.isVideoTab ? 1 : 0.5)
    } else {
  ...
  render() {
    const { navigationState, scrollEnabled, bounces } = this.props;
    ...
    return (
      <Animated.View style={[styles.tabBar, this.props.style]}>
        ...
        <View style={styles.scroll}>
          ...
          >
            {routes.map(this._renderTabButton)}
            {this._renderTabButton({ key: 'third', title: 'Video', name: 'Video', next: 'Next', isVideoTab: true }, 2)}
          </Animated.ScrollView>
        </View>
      </Animated.View>
    );
...
export default recompose(
  connect(
    (state) => {
      const { videoFocused } = FetchingRedux.getReducerState(state)
      return {
        videoFocused
      }
    },
